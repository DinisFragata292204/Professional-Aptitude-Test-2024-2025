...
if ($action === 'resend') {
    ...
    // Ação "update": valida o código e, se correto, insere o email na tabela dadosdoutilizador se ainda não existir
    if (empty($code)) {
    ...
    // Código verificado com sucesso; agora insere o email na tabela dadosdoutilizador se ainda não existir.
    $checkStmt = $conn->prepare("SELECT id FROM dadosdoutilizador WHERE email = ?");
    $checkStmt->bind_param("s", $email);
    $checkStmt->execute();
    $checkStmt->store_result();
    
    if ($checkStmt->num_rows === 0) {
        // Email não existe; insere novo registo com o email e demais campos com valores default
        $insertStmt = $conn->prepare("INSERT INTO dadosdoutilizador (email) VALUES (?)");
        $insertStmt->bind_param("s", $email);
        if (!$insertStmt->execute()) {
            echo json_encode(["success" => false, "message" => "Erro ao inserir email na base de dados: " . $insertStmt->error]);
            $insertStmt->close();
            $checkStmt->close();
            $conn->close();
            exit();
    ...
    // Retorna sucesso e instrução de redirecionamento
    echo json_encode([
        "success" => true,
